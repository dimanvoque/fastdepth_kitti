...
def main():
    ...
    # create new model if checkpoint does not exist
        ...
        if args.arch == 'MobileNetSkipAdd': # if we use skip-add connections
            model = models.MobileNetSkipAdd(output_size=train_loader.dataset.output_size) # MobileNetSkipAdd model is created
        else:
            model = models.MobileNetSkipAdd(output_size=train_loader.dataset.output_size)  # by default we use MobileNetSkipAdd model
        print("=> model created.")
